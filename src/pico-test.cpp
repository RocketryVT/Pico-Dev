...
const char* executeable_name = "pico-test.uf2";
...
int main() {
    ...
    //Sensors are not natively on Picos, so they are removed from this test script
    // alt.initialize();
    // sleep_ms(500);
    // adxl375.initialize();
    // sleep_ms(500);
    ...
    //Sensor polling tasks are not initialized for the same reason as above
